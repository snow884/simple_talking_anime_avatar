...
def get_speaker_video(output_video_path, audio_file_name, length_audio, min_desired_length, fps):
    ...
    model = whisper.load_model("tiny")  # Choose model size: tiny, base, small, medium, large
    data = whisper.transcribe(model, audio, language="en", suppress_tokens=[-1] + number_tokens)
    print(data)
    ...
    for i_frame in range(0,max(length_audio, min_desired_length)):
        ...
        if word_start<=timestamp and word_end>timestamp:
            letter_i = int(len(word_text)/(word_end-word_start)*(timestamp-word_start))
            
            pronunce_letter = word_text[letter_i].lower()
            
            if len(word_text)>letter_i:
                if word_text[letter_i:letter_i+2].lower() in ['sh','th','ee','ch']:
