...
def get_image_speaking(letters='bb'):
    
    mouth_filename = get_filename(letters=letters)

    background = cv2.imread('../character_images/female1/main_image.png', cv2.IMREAD_UNCHANGED)
    overlay = cv2.imread(f'../character_images/female1/{mouth_filename}', cv2.IMREAD_UNCHANGED)
    overlay = cv2.resize(overlay, (140, 140))

    mouth_x=450
    mouth_y=570

    res = overlay_transparent(background, overlay, int(mouth_x-overlay.shape[0]/2), int(mouth_y-overlay.shape[1]/2))
